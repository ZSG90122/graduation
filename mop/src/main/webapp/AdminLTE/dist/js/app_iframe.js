...
var addTabs = function (options) {
    ...
    if (findTabPanel(pageId) == null) {
        ...
        if (options.content) {
            ...
            var $iframe = $("<iframe></iframe>").attr("scrolling","yes").attr("src", options.url).css("width", "100%").attr("frameborder", "no").attr("id", "iframe_" + pageId).addClass("tab_iframe").attr(pageIdField, pageId);
...
//在页面上面直接调用sidebar - menu的方法
 ...
 (function ($) {
    $.fn.topbarMenu = function (options) {
        options = $.extend({}, $.fn.topbarMenu.defaults, options || {});
        var $menu_ul = $(this);
        var level = 0;
        //  target.addClass('nav');
        // target.addClass('nav-list');
        if (options.data) {
        	inittop($menu_ul, options.data, level);
        }
        else {
            if (!options.url) return;
            $.getJSON(options.url, options.param, function (data) {

            	inittop($menu_ul, data, level);
            });
        }

        function inittop($menu_ul, data, level) {
            $.each(data, function (i, item) {
                //如果标签是isHeader
                var $header = $('<li class="header"></li>');
                if (item.isHeader != null && item.isHeader === true) {
                ...
                //a标签
                var $a;
                $a = $('<a data-toggle="dropdown" aria-expanded="false"></a>');
                ...
                if (item.children && item.children.length > 0) {
                    ...
                    var pullSpan = $('<span class="dropdown-toggle"></span>');
                    //var pullIcon = $('<i class="fa fa-angle-left pull-right"></i>');
                    //pullSpan.append(pullIcon);
                    $a.append(pullSpan);
                    li.append($a);
                    ...
                    var menus = $('<ul></ul>');
                    menus.addClass('dropdown-menu');
                    
                    inittop(menus, item.children, level + 1);
                    li.append(menus);
                }
                else {
        ...
        //另外绑定菜单被点击事件,做其它动作
        $menu_ul.on("click", "li.dropdown a", function () {
            var $a = $(this);
