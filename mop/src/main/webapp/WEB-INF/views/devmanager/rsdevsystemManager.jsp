...
<script>
	...
	$(function() {
		...
		$("#btn-add").on("click", function() {
			rchouseid = null;
			sel.syndata('depid', "<%=request.getContextPath()%>/rest/department/getdeplist/0", 'id', 'name',function(data){});
		...
		$("#depid").on('change', function() {
			...
			if (rchouseid != null) {
				//查找相应市州下的遥控站
				sel.binddata('rchouseid', "<%=request.getContextPath()%>/rest/remotestation/getRemoteStationList?owerdep=" + depVal, 'id', 'name', rchouseid);
				rchouseid = null;
			} else {
				sel.bindselectfirst('rchouseid', "<%=request.getContextPath()%>/rest/remotestation/getRemoteStationList?owerdep=" + depVal, 'id', 'name');
			}
		});
