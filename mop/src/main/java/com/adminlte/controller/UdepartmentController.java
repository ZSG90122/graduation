...
public class UdepartmentController extends BaseController {
	...
	public ResponseEntity<DatatablesResult> manager(@RequestParam(value = "draw") String draw,
		...
		try {
			DatatablesResult datatablesResult = this.departmentService.selectDataGrid(page, rows,
	...
	public ResponseEntity<DatatablesResult> enterprisedataGrid(String querystring,
		...
		try {
			...
			DatatablesResult datatablesResult = this.enterpriseService.selectDataGridWrapper(page, rows,
	...
	public Result deleteEnterpriseBatch(@RequestBody List<Benterprise> enterprisesIds) {
		...
		try {
			...
			// enterpriseService.deleteBatchIds(enterprisesIds);
			// //要这个函数，似乎表不用id，用tbid等或许可以
			return new Result(true);
		} catch (Exception ex) {
			System.out.println(ex.toString());
	...
	/*
	 * by CGP 根据传入的departmentId数组 获取udepartment表中对应departmentId的联系人的名字
	 */
	@RequestMapping(value = "/getContactsName/{data}", method = RequestMethod.POST)
	@ResponseBody
	public ResponseEntity<DatatablesResult> getContactsName(@PathVariable("data") String data,
			@RequestParam(value = "draw") String draw, @RequestParam(value = "start") String start,
			@RequestParam(value = "length") String length) {
		int rows = Integer.parseInt(length);
		int page = (Integer.parseInt(start) / rows) + 1;
	...
	@ResponseBody
	public List<Udepartment> getdeplist() {
		EntityWrapper<Udepartment> wrapper = new EntityWrapper<Udepartment>();
		wrapper.eq("parentid", 1);
		wrapper.orderBy("id");
		return departmentService.selectList(wrapper);
	}
